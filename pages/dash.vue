  ...
  <div v-if="$store.state.authUser">
    ...
    <div v-if="$store.state.authUser.userStatus==1" class="pads">
      <div>
        ...
        <el-row :gutter="20">
          <el-col class :span="12">
            <el-card class="box-card" v-loading="dataLoads.loading.panels">
              ...
              <div class="text item">Total views: {{stats.totalViews}}</div>
              <div class="text item">Active videos: {{videos.length}}</div>
            </el-card>
          </el-col>
          <el-col class :span="12">
            <el-card class="box-card" v-loading="dataLoads.loading.panels">
              ...
              <div class="text item">Total users registered: {{stats.userCount}}</div>
              <div class="text item">Total videos uploaded: {{stats.videoCount}}</div>
              <div class="text item">Max storage space: {{stats.totalSpaceA*0.000001}} MB</div>
              <div
                class="text item"
              >Space used: {{stats.usedSpaceA}} / {{stats.totalSpaceA*0.000001}} MB</div>
            </el-card>
          </el-col>
        </el-row>
        <!-- Global video table -->
        <el-table
          :data="videos"
          v-loading="dataLoads.loading.videoList"
        ...
        >
          ...
          <el-table-column prop="name" label="Video">
            <template slot-scope="scope">
              <div class="nameColumn">{{videos[scope.$index].name}}</div>
            </template>
          </el-table-column>
        ...
        <el-card>
          <el-button
            ...
            type="danger"
            size="small"
            @click.native.prevent="deleteVideo(multipleSelection)"
    ...
    <div v-else>
      <div
        v-if="videos.length==0 && searchTerm=='' && hasVideos==false && !dataLoads.loading.videoList"
        class="centeredUploadVideoSuggestion"
      >
        <el-card>
          <div v-if="$store.state.authUser.accountStanding==2">
            <p style="display:block;">Suspended.</p>
            <el-button @click="upgradeInit">Use code</el-button>
          </div>
          <div v-else>
      ...
      <div class="videoList" v-else>
        ...
        <div class="cards">
          ...
          <el-card class="box-card statCard" v-loading="dataLoads.loading.panels">
            ...
            <div class="text item">Total storage space: {{stats.totalSpace}} MB</div>
            <div class="text item">Space used: {{stats.usedSpace}} / {{stats.totalSpace}} MB</div>
          </el-card>
          <el-card class="box-card statCard" v-loading="dataLoads.loading.panels">
            <div slot="header" class="clearfix">
            ...
            <div v-if="$store.state.authUser.accountStanding===0">
              <div class="text item">
                ...
                <i
                  ...
                  style="color:#98FB98; -webkit-text-stroke: 2px white;"
                  aria-hidden="true"
                ></i>
              </div>
            </div>
            ...
            <div v-else-if="$store.state.authUser.accountStanding===1">
              <div class="text item">
                ...
                <el-tooltip
                  class="item"
                  effect="light"
                  content="Some of your videos have been deleted because they contained forbidden content."
                  placement="top-start"
                >
            ...
            <div v-else-if="$store.state.authUser.accountStanding===2">
              <div class="text item">
                ...
                <el-tooltip
                  class="item"
                  effect="light"
                  content="An admin forbade you from uploading new videos."
                  placement="top-start"
                >
        ...
        <el-card>
          <el-button
            ...
            type="warning"
            size="medium"
            @click.native.prevent="requestNewID(multipleSelection)"
          >New links for selected</el-button>
          <el-button
            :disabled="multipleSelection.length==0"
            type="danger"
            size="medium"
          ...
          >
            ...
            <el-table-column prop="name" label="Name">
              <template slot-scope="scope">
                <div class="nameColumn">
                  ...
                  <i
                    class="fa fa-pencil fa-lg renameIcon"
                    aria-hidden="false"
                    @click="requestNewName(scope.$index)"
                  ></i>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="Link" @click.native="$event.target.select()" width="250">
              <template slot-scope="scope">
                <div class="linkColumn">
                  <a :href="videos[scope.$index].link">{{videos[scope.$index].link}}</a>
                  <el-tooltip
                    :content="currentCopyTooltip"
                    :enterable="false"
                    transition="el-zoom-in-top"
                  >
                    <i
                      class="fa fa-clipboard fa-lg copyIcon"
                      aria-hidden="false"
                      @click="copyLink(videos[scope.$index].link)"
                    ></i>
                  </el-tooltip>
                </div>
              </template>
            </el-table-column>
            <el-table-column prop="views" sortable label="Views" width="100"></el-table-column>
            <el-table-column label="Ratings">
              <template slot-scope="scope" class="ratingColumn">
                ...
                <i
                  class="fa fa-thumbs-up fa-rotate-180"
                  style="color:red;"
                  aria-hidden="true"
                ></i>
              </template>
                ...
                <el-form size="small">
                  <el-form-item>
                    <el-button
                      ...
                      type="warning"
                      size="small"
                      @click.native.prevent="requestNewID([videos[scope.$index]])"
                    >New link</el-button>
                  </el-form-item>
                  <el-form-item>
                    <el-button
                      ...
                      type="danger"
                      size="small"
                      @click.native.prevent="deleteVideo([videos[scope.$index]])"
                    >Remove</el-button>
                  </el-form-item>
                </el-form>
              </template>
